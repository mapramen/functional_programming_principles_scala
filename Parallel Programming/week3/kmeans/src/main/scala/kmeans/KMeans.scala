...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    def addMissingMean(meansMap: Map[Point, Seq[Point]], mean: Point): Map[Point, Seq[Point]]
      = if (meansMap.contains(mean)) meansMap else meansMap ++ Seq((mean, Seq.empty[Point]))
    
    val initialMeansMap: Map[Point, Seq[Point]] = points.groupBy(findClosest(_, means))

    means.foldLeft(initialMeansMap)((meansMap, mean) => addMissingMean(meansMap, mean))
  }

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    def addMissingMean(meansMap: ParMap[Point, ParSeq[Point]], mean: Point): ParMap[Point, ParSeq[Point]]
      = if (meansMap.contains(mean)) meansMap else meansMap + ((mean, Seq.empty[Point].par))
    
    val initialMeansMap: ParMap[Point, ParSeq[Point]] = points.groupBy(findClosest(_, means))
