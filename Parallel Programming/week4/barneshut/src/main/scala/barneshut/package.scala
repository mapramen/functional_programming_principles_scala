...
package object barneshut {
  ...
  case class Fork(nw: Quad, ne: Quad, sw: Quad, se: Quad) extends Quad {
    ...
    def insert(b: Body): Fork = {
      val inNorth = b.y < centerY
      val inWest = b.x < centerX

      (inNorth, inWest) match {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: coll.Seq[Body]) extends Quad {
    val mass: Float = bodies.foldLeft(0.0f)((s, b) => s + b.mass)
    val massX: Float = bodies.foldLeft(0.0f)((s, b) => s + b.mass * b.x) / mass
    val massY: Float = bodies.foldLeft(0.0f)((s, b) => s + b.mass * b.y) / mass
    ...
    def insert(b: Body): Quad = {
      if(size <= minimumSize){
          ...
          val forkSize: Float = size / 2
          val initialFork: Quad = Fork(
            Empty(centerX - forkSize / 2, centerY - forkSize / 2, forkSize),
            Empty(centerX + forkSize / 2, centerY - forkSize / 2, forkSize),
            Empty(centerX - forkSize / 2, centerY + forkSize / 2, forkSize),
            Empty(centerX + forkSize / 2, centerY + forkSize / 2, forkSize)
          )

          bodies.foldLeft(initialFork.insert(b))((fork, body) => fork.insert(body))
        }
    }
  }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface {
    ...
    def getIndex(x: Float, l: Float): Int
      = scala.math.max(0, scala.math.min(sectorPrecision - 1, ((x - l) / sectorSize).toInt))

    def +=(b: Body): SectorMatrix = {
      val x: Int = getIndex(b.x, boundaries.minX)
      val y: Int = getIndex(b.y, boundaries.minY)
