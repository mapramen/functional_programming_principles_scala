...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromPartialFunction { case data: Json.Num if data.value.isValidInt => data.value.intValue }

  /** A decoder for `String` values */
  implicit val stringDecoder: Decoder[String] =
    Decoder.fromPartialFunction { case data: Json.Str => data.value }

  /** A decoder for `Boolean` values */
  implicit val booleanDecoder: Decoder[Boolean] =
    Decoder.fromPartialFunction { case data: Json.Bool => data.value }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case jsonArray: Json.Arr => {
        lazy val decodedList: List[Option[A]] = jsonArray.items.map(decoder.decode)
        if (decodedList.exists(_ == None)) None else Some(decodedList.flatten)
      }
      case everythingElse => None
    }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case jsonObj: Json.Obj if jsonObj.fields.contains(name) => decoder.decode(jsonObj.fields(name))
      case everythingElse => None
    }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder.field[String]("name")
      ...
      .transform[Person]({ case (name: String, age: Int) => Person(name, age) })
